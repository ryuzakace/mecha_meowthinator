...
def loadKNNDataAndTrainKNN():
    ...
    npaFlattenedImages = np.loadtxt(BASE_DIR+'/core/flattened_images.txt', np.float32)                 # read in training images
    npaClassifications = np.loadtxt(BASE_DIR+'/core/classifications.txt', np.float32)                  # read in training classifications
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        if (len(listOfListsOfMatchingCharsInPlate) == 0):# if no groups of matching chars were found in the plat
